<?php
	...
	Class datasourceforum_read_discussions_bookmarked extends Datasource{

		var $dsParamROOTELEMENT = 'forum-read-discussions';

		var $dsParamFILTERS = array(
				'id' => '{$ds-forum-discussions-bookmarked}',
		);
		...
		function about(){
			return array(
					 'name' => 'Forum: Read Discussions List (Bookmarked)',
					 'author' => array(
							'name' => 'Symphony Team',
		...
		function grab(&$param_pool){
			...
			if(!$members->isLoggedIn() || !isset($param_pool['ds-forum-discussions-bookmarked']) || empty($param_pool['ds-forum-discussions-bookmarked']))
				...
				$pre_dated_discussions = array();
				$discussions = $param_pool['ds-forum-discussions-bookmarked'];
				$discussions = array_filter($discussions, 'strlen');
				...
				if (!empty($discussions))
				{
					$pre_dated_discussions = Symphony::Database()->fetchCol('entry_id',
						sprintf(
							...
							WHERE `entry_id` IN (".@implode(',', $discussions).")
							AND `local` <= '%s'",
				...
				$read_discussions = @implode(',', array_diff($param_pool['ds-forum-discussions-bookmarked'], $pre_dated_discussions));
